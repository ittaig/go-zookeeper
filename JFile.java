...
public class JFile {
    ...
    public void genCode( String language, File outputDirectory )
        ...
        if( "c++".equals( language ) ) {
            CppGenerator gen =
                new CppGenerator( mName, mInclFiles, mRecords,
                    outputDirectory );
            gen.genCode();
        } else if( "java".equals( language ) ) {
            JavaGenerator gen =
                new JavaGenerator( mName, mInclFiles, mRecords,
                    outputDirectory );
            gen.genCode();
        } else if( "c".equals( language ) ) {
            CGenerator gen =
                new CGenerator( mName, mInclFiles, mRecords, outputDirectory );
            gen.genCode();
        } else if( "csharp".equals( language ) ) {
            final CSharpGenerator gen =
                new CSharpGenerator( mName, mInclFiles, mRecords,
                    outputDirectory );
            gen.genCode();
        } else if( "go".equals( language ) ) {
            final GoGenerator gen =
                new GoGenerator( mName, mInclFiles, mRecords, outputDirectory );
            gen.genCode();
        } else {
