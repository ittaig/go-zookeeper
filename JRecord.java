...
public class JRecord extends JCompType {
    ...
    public JRecord( String name, ArrayList<JField> flist ) {
        super( "struct " + name.substring( name.lastIndexOf( '.' ) + 1 ),
            ...
            name.substring( name.lastIndexOf( '.' ) + 1 ), "Record", name,
    ...
    public void genCCode( FileWriter h, FileWriter c ) throws IOException {
        ...
        for( JField f : mFields ) {
            if( f.getType() instanceof JRecord ) {
                ...
                c.write( "    deallocate_"
                    + JVector.extractVectorName( vt.getElementType() ) + "(&v->"
                    + f.getName() + ");\n" );
    ...
    public void genCppCode( FileWriter hh, FileWriter cc ) throws IOException {
        ...
        for( Iterator<JField> i = mFields.iterator(); i.hasNext(); fIdx++ ) {
            ...
            if( jf.getType() instanceof JBuffer ) {
                cc.write( "  a_.serialize(m" + name + ",m" + name
                    + ".length(),\"" + jf.getTag() + "\");\n" );
            } else {
        ...
        for( Iterator<JField> i = mFields.iterator(); i.hasNext(); fIdx++ ) {
            ...
            if( jf.getType() instanceof JBuffer ) {
                cc.write( "  { size_t len=0; a_.deserialize(m" + name
                    + ",len,\"" + jf.getTag() + "\");}\n" );
            } else {
    ...
    public void genCsharpCode( File outputDirectory ) throws IOException {
        ...
        cs.write(
            "  public void ReadFields(MiscUtil.IO.EndianBinaryReader reader) {\n" );
        cs.write(
    ...
    private static String changeAllButFirst(String orig, String regex, String first, String rest) {
    	String ret = orig.replaceFirst(regex, "#");
    	ret = ret.replaceAll(regex, rest);
    	ret = ret.replaceFirst("#", first);
    	return ret;
    }

    private static String removeNFirst(int n, String source) {
    ...
    public void genGoCode( File outputDirectory ) throws IOException {
        String pkg     = getGoPackage();
        //String pkgPath = changeAllButFirst(pkg, "\\.", ".", "/");  //pkg.replaceFirst("\\.", "#");
        final String pkgPath = removeNFirst(3, pkg);
        ...
        File       jfile = new File( pkgdir, getName() + ".go" );
        FileWriter jj    = new FileWriter( jfile );
        jj.write(
                "// File generated by hadoop record compiler. Do not edit.\n" );
        jj.write(
                "// (Gen Date: " + new Date().toString() + ").\n" );
        jj.write( "//\n" );
        jj.write(
            "// Licensed to the Apache Software Foundation (ASF) under one\n" );
        jj.write(
        ...
        jj.write( "\n" );
        jj.write( "package " + pkg.substring( pkg.lastIndexOf( '.' ) + 1 )
            + "\n\n" );
        jj.write( "import \"github.com/ParsePlatform/go.zookeeper/binaryarchives\"\n\n" );

        final Set<String> seenImport = new HashSet<>();
        
        for( JField jf : mFields ) {
            if( jf.getType() instanceof JRecord ) {
                final JRecord jr = ( JRecord )jf.getType();

                if( !jr.mModule.equals( mModule ) && !seenImport.contains(jr.mModule)) {
                    jj.write( "import \"github.com/ParsePlatform/go.zookeeper/generated/" );
                    jj.write( removeNFirst(3, jr.mModule));
                    jj.write( "\"\n" );
                    seenImport.add(jr.mModule);
                }
            } else if( jf.getType() instanceof JVector ) {
                final JVector jv = ( JVector )jf.getType();

                if( jv.getElementType() instanceof JRecord ) {
                    jv.setContainer( this );

                    final JRecord jr = ( JRecord )jv.getElementType();
        ...
        jj.write( "type " + getName() + " struct {\n" );

        for( Iterator<JField> i = mFields.iterator(); i.hasNext(); ) {
            JField jf = i.next();
            jj.write( jf.genGoDecl( this ) );
        }
    ...
    String genCsharpReadMethod( String fname, String tag ) {
        // return "    "+capitalize(fname)+"=a_.Read"+mMethodSuffix+"(" +
        // capitalize(fname) + ",\""+tag+"\");\n";
        return genCsharpReadWrapper( capitalize( fname ), tag, false );
    }

    /*String genGoReadMethod( String fname, String indent, String prefix ) {
        final StringBuilder sb = new StringBuilder();

        sb.append( indent + "  if err == nil {\n" );
        sb.append( indent + "    " );
        sb.append( prefix );
        sb.append( capitalize( fname ) );
        sb.append( " = " );
        sb.append( getGoType( this ) );
        sb.append( "{}\n" );
        sb.append( indent + "    err = " );
        sb.append( prefix );
        sb.append( capitalize( fname ) );
    ...
    String genGoWriteMethod( String fname, String indent, String prefix ) {
        final StringBuilder sb = new StringBuilder();
    ...
    private void genDeserialize( FileWriter c, JType type, String tag,
        ...
        if( type instanceof JRecord ) {
            c.write( "    rc = rc ? rc : deserialize_"
                + extractStructName( type ) + "(in, \"" + tag + "\", &v->"
                + name + ");\n" );
        } else if( type instanceof JVector ) {
            c.write( "    rc = rc ? rc : deserialize_"
                + JVector.extractVectorName(
                    ( ( JVector )type ).getElementType() ) + "(in, \"" + tag
                + "\", &v->" + name + ");\n" );
        } else {
    ...
    private void genSerialize( FileWriter c, JType type, String tag,
            String name ) throws IOException {
        if( type instanceof JRecord ) {
            ...
            c.write( "    rc = rc ? rc : serialize_"
                + JVector.extractVectorName(
                    ( ( JVector )type ).getElementType() ) + "(out, \"" + tag
                + "\", &v->" + name + ");\n" );
        } else {
