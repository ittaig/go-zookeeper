...
abstract class JCompType extends JType {
     ...
     */
    JCompType( String cType, String cppType, String csharpType, String javaType,
            String goType, String suffix, String wrapper,
            String csharpWrapper ) {
        super( cType, cppType, csharpType, javaType, goType, suffix, wrapper,
            csharpWrapper, null );
    }

    String genCppGetSet( String fname, int fIdx ) {
        String cgetFunc =
            "  virtual const " + getCppType() + "& get" + fname
            + "() const {\n";
        cgetFunc += "    return m" + fname + ";\n";
        cgetFunc += "  }\n";

        String getFunc =
            "  virtual " + getCppType() + "& get" + fname + "() {\n";
    ...
    String genCsharpCompareTo( String name ) {
        return "    ret = " + capitalize( name ) + ".CompareTo(peer."
        + capitalize( name ) + ");\n";
    }
    ...
    String genCsharpEquals( String name, String peer ) {
        ...
        return "    ret = " + capitalize( name ) + ".Equals(" + peerSplit[ 0 ]
